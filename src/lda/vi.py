...
class VariationalBayes(object):
    # 
    def __init__(self, gamma_converge=0.000001, gamma_maximum_iteration=400, alpha_converge=0.000001, alpha_maximum_iteration=100, em_maximum_iteration = 5, em_converge = 0.00001):
    ...
    @param data: a defaultdict(dict) data type, first indexed by doc id, then indexed by term id, the value is the appearance of that term in that doc.
    @note: words are not terms, they are repeatable and thus might be not unique
    """
...
if __name__ == "__main__":
    ...
    d = import_monolingual_data("../../data/test/doc.txt");
