...
"""
    ...
    def run(self, matrix, maxiter=30):
        ...
        for it in xrange(maxiter):
            for doc_id in xrange(n_docs):
                for i, word_id in enumerate(word_indices(matrix[doc_id, :])):
                    topic_id = self.topics[(doc_id,i)]
                    self.nmz[doc_id,topic_id] -= 1
                    self.nm[doc_id] -= 1
                    self.nzw[topic_id,word_id] -= 1
                    self.nz[topic_id] -= 1

                    p_z = self._conditional_distribution(doc_id, word_id)
                    topic_id = sample_index(p_z)

                    self.nmz[doc_id,topic_id] += 1
                    self.nm[doc_id] += 1
                    self.nzw[topic_id,word_id] += 1
                    self.nz[topic_id] += 1
                    self.topics[(doc_id,i)] = topic_id
